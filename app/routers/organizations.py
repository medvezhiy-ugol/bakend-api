...
async def get_organizations_mongo(session_mongo: AsyncIOMotorClientSession = Depends(get_mongo_session)):
    res = await get_organization_mongo(session_mongo)
    return JSONResponse(res)


@organization_router.post("/organizations",status_code=status.HTTP_200_OK)
async def get_organizations(token: str = Depends(get_token_iiko),
                            session_mongo: AsyncIOMotorClientSession = Depends(get_mongo_session)):
    resp = await IIko().get_organiztions(token)
