...
@terminal_router.post("/terminal", status_code=status.HTTP_200_OK)
async def get_all_terminals(
    Id: TerminalOneModel = Body(...), token: str = Depends(get_token_iiko)
):
    terms = await TerminalResponse.get(Id.organizationId)
    return terms


@terminal_router.post("/terminalIIko", status_code=status.HTTP_200_OK)
async def get_iiko_term(
    term: TerminalModel = Body(...),
    ...
    new_terms = TerminalResponse(
        id=term.organizationIds[0],
